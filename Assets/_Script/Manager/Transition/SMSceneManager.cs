...
public delegate void SceneAction(string id);
/// <summary>
/// This class is the main interaction point with the SceneManager API.
/// </summary> 
public class SMSceneManager : SingletonBehaviour<SMSceneManager>
{
	
	private SMILevelProgress levelProgress;
	public SMIConfigurationAdapter configurationAdapter;
	
	string defaultTransitionPrefab = "Transitions/SMFadeTransition";
    public string currentLevel;
    public event SceneAction SceneLoaded;
	/// <summary>
    ...
    public void LoadLevel(int sceneId, string transitionPrefab = PrefabsPath.SMProgressTransition)
    {
        SceneInfo si = ResManager.Instance.sceneTable.GetById(sceneId);
        if (si != null)
        {
            string name = si.name;
            //if (  !configurationAdapter.LevelExists(levelId) ) {
            ...
            //}
            LoadScreen(name, transitionPrefab);
        }
	}
    /// <summary>
    ...
    void OnSceneLoaded(string id)
    {

        if (id == MapId.MapCity)
        {
            Debug.Log("back home scene loaded, id : " + id);
        }
        else
        {
            Debug.Log("  scene loaded, id : " + id);

        }
        SMTransition.state = SMTransitionState.None;
        if (SceneLoaded != null)
        {
            SceneLoaded(id);
        }
